...
const AISidebar = ({
  ...
  const handleGenerateSummary = async () => {
    const jsonString = JSON.stringify(textEditorValue, null, 2);

    const markdownContent = `## Apprvd\n\nText Editor Value:\n\n\`\`\`json\n${jsonString}\n\`\`\`\n`;

    const markdownBlob = new Blob([markdownContent], {
      type: "text/markdown",
    });
    const markdownFile = new File([markdownBlob], "data.md", {
      type: "text/markdown",
    });

    const formData = new FormData();
    formData.append("prompt", reviewInput);
    ...
    <div className="text-sm pt-4">
      <div className="bg-white p-6 rounded-lg shadow-md max-w-lg mx-auto">
        ...
        <div className="text-center">
          <button className="bg-blue-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-blue-700 focus:outline-none focus:ring focus:bg-blue-500">
            Accept suggestions
          </button>
        </div>
      </div>
