...
const HomeNavbar: React.FC<HomeNavbarProps> = ({ navItems }) => {
    ...
    return (
        <nav className="top-0 left-0 right-0 w-full flex items-center justify-between flex-wrap bg-secondary p-8">
            ...
            <div className={`w-full flex-grow lg:flex lg:items-center lg:w-auto ${navBarOpen ? 'block' : 'hidden'}`}>
                ...
                <div>
                    <a href="#" className="inline-block text-sm px-16 py-4 leading-none border rounded text-white border-white mt-4 lg:mt-0 hover:border-transparent hover:text-white hover:bg-primary transition-colors duration-500">Try Now</a>
                </div>
            </div>
        </nav>
    )
