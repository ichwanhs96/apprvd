...
class IndentSpacingConv {
    static from(format: any, conv: any) {
    ...
    static to(format: any, conv: any) {
        for(let pr of indentSpacingPrs) {
            if (conv[pr])
                format[pr] = new Pt(conv[pr]);
        }
    }
    static indent(conv: any, styleObj: any){
...
class BorderConv {
    ...
    static side2styleObj(conv: any, styleObj: any, side: any){
